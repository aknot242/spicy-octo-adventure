...
resource "aws_iam_role" "eks-iam-role" {
  name = format("%s-eks-iam-role-%s", var.project_prefix, var.build_suffix)
...
resource "aws_iam_role" "workernodes" {
  name = format("%s-eks-node-iam-role-%s", var.project_prefix, var.build_suffix)
...
resource "aws_iam_policy" "workernodes_ebs_policy" {
  name = format("%s-ebs_csi_driver-%s", var.project_prefix, var.build_suffix)
...
resource "aws_iam_role" "ebs_csi_driver" {
  name               = format("%s-ebs-csi-driver-%s", var.project_prefix, var.build_suffix)
